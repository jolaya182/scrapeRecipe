...
(async () => {
  ...
  for(let i = 0; i < textCsv.length; i += 1){
    ...
    //   // headers.set(
     ...
     await page.goto(data[0], {waitUntil:'networkidle2'}).catch(err=>console.log("err going to the intial page", err));
      ...
      // await console.log("page is ", page);
      // await page.goto("http://www.guitarflavors.com/j/j.html");
    // await page.goto("file:///C:/scrape/hired/.hired.html");
    // await page.goto("https://www.glassdoor.com/job-listing/fullstack-engineer-daily-muse-JV_IC1132348_KO0,18_KE19,29.htm?jl=2625634168");
  ...
  for(let i = 1; i < pages.length; i += 1){
    ...
    let aryUrlName ;
    if( url.indexOf( "angel" ) !== -1 ) aryUrlName = "angel";
      if( url.indexOf( "vettery" ) !== -1 ) aryUrlName = "vettery";
        else if(url.indexOf("stackoverflow") !== -1) aryUrlName = "stackoverflow";
          else if(url.indexOf("builtinnyc") !== -1) aryUrlName = "builtinnyc";
            else if(url.indexOf("linkedin") !== -1) aryUrlName = "linkedin";
    ...
    if(title.site == "angel" ){ 
      ...
      // await p.waitForNavigation().catch(err => console.log("error waitForNavigation",err));
   }
     ...
     if(co != "")  await p.waitForSelector(co).catch(err=>console.log("error for name", err));
      ...
      let t = { 
        t : title.t == ""?"": ti,   //await p.evaluate( (ti)=> document.querySelector(ti), ti )
        s : title.s == ""?"": sk,  //await p.evaluate( (sk)=> document.querySelector(sk), sk )
        n : title.n == ""?"": na, //await p.evaluate( (na)=> document.querySelector(na), na ) //check for click on apply now
        c : title.c == ""?"": co,
        site: title.site   }
  ...
  //     // return  scrapedItems ;
  //     // return title;
  //   // },{t:recipe[ aryUrlName ].title, s: recipe[ aryUrlName ].skillToScrape, n: recipe[ aryUrlName].nameToScrape, site: aryUrlName} );    
    //test purposes
    ...
    // jobTitle h1
    // companyName h1
    //body#layouts-base-body > div > div:nth-of-type(2) > div:nth-of-type(2) > div > div:nth-of-type(2) > h1
  ...
  companyName = titleSkillName.c.replace(",", " ").replace(";", " ") || "n/a" ;
    // skillsR body#layouts-base-body > div > div:nth-of-type(3) > div > div:nth-of-type(3) > div:nth-of-type(2)
  ...
  skillsR =titleSkillName.s.replace(",", " ").replace(";", " ") || "n/a" ;
    ...
    // let req = "";
    // contactPerson body#layouts-base-body > div:nth-of-type(7) > div > div > div > div > div:nth-of-type(3) > div:nth-of-type(2) > div > div > div > div:nth-of-type(3) > div:nth-of-type(3) > a:nth-of-type(2)
  ...
  .then( async function (response) {
    ...
    console.log('Name: ', response.name);
      ...
      let contcts = await clearbit.Prospector.search({domain: response.domain, limit:20 })
...
function getNameFromUrl(s){
  // s = "https://angel.co/algorithmia?src=news";
  // s = "https://www.guitarflavors.com/j/j.html";
  // s = "C:/scrape/j.html";
  let str = s.slice(8);
  // c(str);
  let strA = str.split(".");
...
function processScrapDataBySite(title){
